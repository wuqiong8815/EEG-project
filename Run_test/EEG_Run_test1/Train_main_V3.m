...
% 4.5 新预处理数据下的敏感通道诊断
% =========================================================
fprintf('Step 4: Channel sensitivity diagnostics on current trialSplit data...\n');
...
%% =========================================================
% 5  Critical-K 诊断（分类器通道鲁棒性，仅前向，不重建）
% =========================================================
fprintf('Step 5: Running Critical-K diagnostic...\n');

criticalFile = fullfile('Train', 'critical_K.mat');
criticalPng  = fullfile('Train', 'critical_K.png');

K_max_scan    = min(10, C_eeg);
num_trials_ck = 30;
drop_threshold = 0.03;
seed_ck = 2026;
classifier_tag = 'EEGNet-lossClassifierNet';

criticalInfo = runCriticalKDiagnostic( ...
    F_feat, ...
    Y_train, ...
    lossClassifierNet, ...
    K_max_scan, ...
...
fprintf('Critical-K done: Critical_K=%g, Critical_K_worst=%g, K_max_rec=%g\n', ...
    criticalInfo.Critical_K, criticalInfo.Critical_K_worst, criticalInfo.K_max_rec);
fprintf('Saved: %s\n', criticalFile);
fprintf('Saved: %s\n', criticalPng);
...
[W1, b1, W2, b2, E_ch, E_mask, history] = Train_Generator( ...
    F_feat, ...
    A_repair, ...
    Y_train, ...
    lossClassifierNet, ...
...
for evalCase = 1:numCases
    ...
    for rr = 1:numEvalRepeats
        ...
        gen_params_eval = struct();
        gen_params_eval.W1 = W1;
        gen_params_eval.b1 = b1;
        gen_params_eval.W2 = W2;
        gen_params_eval.b2 = b2;
        gen_params_eval.E_ch = E_ch;
        gen_params_eval.E_mask = E_mask;
        gen_params_eval.useContext = false;
...
% 修复结果
gen_params_eval2 = struct();
gen_params_eval2.W1 = W1;
gen_params_eval2.b1 = b1;
gen_params_eval2.W2 = W2;
gen_params_eval2.b2 = b2;
gen_params_eval2.E_ch = E_ch;
gen_params_eval2.E_mask = E_mask;
gen_params_eval2.useContext = false;
...
% ===========================================================
function [W1, b1, W2, b2, E_ch, E_mask, history] = Train_Generator( ...
    F_normal, ...
    A, ...
    Y_train, ...
    lossClassifierNet, ...
...
% ===== K 区间来源：critical_K.mat + 手动上界覆盖 =====
criticalFile = fullfile('Train', 'critical_K.mat');
useCritical = exist(criticalFile, 'file') == 2;

if useCritical
    Sck = load(criticalFile);

    if ~isfield(Sck, 'Critical_K') || ~isfield(Sck, 'K_max_rec')
        error('critical_K.mat exists but missing Critical_K or K_max_rec.');
    end

    K_min = double(Sck.Critical_K);
    if isnan(K_min) || K_min < 1
        K_min = 3;
    end

    K_max_rec = double(Sck.K_max_rec);
    if isnan(K_max_rec) || K_max_rec < 1
        K_max_rec = min(ceil(0.25 * C), 6);
    end

    if nargin >= 11 && ~isempty(K_max) && K_max > 0
        K_max_train = double(K_max);
        fprintf('Train_Generator: manual K_max override = %d (critical K_min kept)\n', K_max_train);
    else
        K_max_train = K_max_rec;
    end
else
    K_min = 3;
    K_max_train = min(ceil(0.25 * C), 6);
    warning('critical_K.mat not found. Fallback to default K range [%d, %d].', K_min, K_max_train);

    if nargin >= 11 && ~isempty(K_max) && K_max > 0
        K_max_train = double(K_max);
        fprintf('Train_Generator: manual K_max override = %d (fallback K_min=%d)\n', K_max_train, K_min);
    end
end

K_min = max(1, min(C, round(K_min)));
K_max_train = max(1, min(C, round(K_max_train)));

if K_min > K_max_train
    error('Invalid K range: K_min=%d > K_max_train=%d.', K_min, K_max_train);
end

fprintf('Train_Generator K range used: [%d, %d]\n', K_min, K_max_train);

% ===== 初始化 =====
unitsPerEpoch = 6;
batchesPerUnit = 2;
numIters = unitsPerEpoch * batchesPerUnit;
plotEvery = 5;

E_ch_dim = 8;
E_mask_dim = 8;
D_in = T + E_ch_dim + E_mask_dim;   % 改动①a

W1 = dlarray(single(randn(D_in, H) * sqrt(2 / D_in)));
b1 = dlarray(single(zeros(1, H)));
W2 = dlarray(single(zeros(H, T)));
b2 = dlarray(single(zeros(1, T)));

% 改动①：条件嵌入
E_ch   = dlarray(single(randn(E_ch_dim,   C) * 0.01));
E_mask = dlarray(single(randn(E_mask_dim, C) * 0.01));
...
history.k_used = zeros(max_epoch, 1);
history.lr_used = learningRate * ones(max_epoch, 1);
history.stage = strings(max_epoch, 1);

history.K_min = K_min;
history.K_max_train = K_max_train;
history.numIters = numIters;

% ===== figure =====
fig = figure('Name', 'Route1 Generator Monitor', 'Color', 'w');
...
ax2 = nexttile; title(ax2, 'Recon Loss per Channel'); xlabel(ax2, 'Epoch'); ylabel(ax2, 'Channel');
ax3 = nexttile; title(ax3, 'Channel Visits'); xlabel(ax3, 'Epoch'); ylabel(ax3, 'Channel');
...
ax5 = nexttile; title(ax5, 'Mean Channel Recon'); xlabel(ax5, 'Channel'); ylabel(ax5, 'Recon Loss'); grid(ax5, 'on');
ax6 = nexttile; title(ax6, 'Total Channel Visits'); xlabel(ax6, 'Channel'); ylabel(ax6, 'Visits'); grid(ax6, 'on');

% ===== train =====
for epoch = 1:max_epoch

    history.stage(epoch) = "Route1-Krange";
    ...
    for it = 1:numIters
        % 改动②：区间随机 K
        K_cur = randi([K_min, K_max_train]);
        abnormal_channels = randperm(C, K_cur);  % 均匀采样通道
        epoch_k_sum = epoch_k_sum + K_cur;
        ...
        [loss, logit_loss, feature_loss, recon_abnormal_loss, label_ce_loss, recon_loss_channel, ...
            grad_W1, grad_b1, grad_W2, grad_b2, grad_E_ch, grad_E_mask] = dlfeval( ...
            @modelGradients, ...
            F_batch, ...
            dlarray(abnormal_mask), ...
            A_dl, ...
            W1, b1, W2, b2, E_ch, E_mask, ...
            lossClassifierNet, ...
            labels_batch_dl, ...
    ...
    history.k_used(epoch) = epoch_k_sum / max(epoch_steps, 1);
    ...
    fprintf(['Epoch %d/%d [Route1 K-range], K~[%.2f], Iters=%d, ' ...
             'Total=%.6f, Logit=%.6f, FEAT=%.6f, RECON=%.6f, LabelCE=%.6f\n'], ...
        epoch, max_epoch, history.k_used(epoch), numIters, ...
    ...
    if mod(epoch, plotEvery) == 0 || epoch == 1 || epoch == max_epoch
        ...
        xlabel(ax2, 'Epoch'); ylabel(ax2, 'Channel'); title(ax2, 'Recon Loss per Channel');

        cla(ax3);
        imagesc(ax3, history.channel_visits(1:epoch, :)');
        axis(ax3, 'xy'); colorbar(ax3);
        xlabel(ax3, 'Epoch'); ylabel(ax3, 'Channel'); title(ax3, 'Channel Visits');
        ...
        xlabel(ax5, 'Channel'); ylabel(ax5, 'Recon Loss'); title(ax5, 'Mean Channel Recon'); grid(ax5, 'on');

        cla(ax6);
        bar(ax6, sum(history.channel_visits(1:epoch, :), 1));
        xlabel(ax6, 'Channel'); ylabel(ax6, 'Visits'); title(ax6, 'Total Channel Visits'); grid(ax6, 'on');
...
W2 = extractdata(W2);
b2 = extractdata(b2);
E_ch = extractdata(E_ch);
E_mask = extractdata(E_mask);

end

%% 梯度更新
function [loss, logit_loss, feature_loss, recon_abnormal_loss, label_ce_loss, recon_loss_channel, ...
    grad_W1, grad_b1, grad_W2, grad_b2, grad_E_ch, grad_E_mask] = modelGradients( ...
    F_normal, ...
    abnormal_mask, ...
    A, ...
    W1, ...
...
gen_params.W2 = W2;
gen_params.b2 = b2;
gen_params.E_ch = E_ch;
gen_params.E_mask = E_mask;
gen_params.useContext = false;
...
[grad_W1, grad_b1, grad_W2, grad_b2, grad_E_ch, grad_E_mask] = dlgradient( ...
    loss, W1, b1, W2, b2, E_ch, E_mask);
...
%% Critical-K 诊断函数
function criticalInfo = runCriticalKDiagnostic( ...
    X_val, Y_val, classifierNet, ...
    K_max_scan, num_trials, drop_threshold, ...
    saveMatPath, savePngPath, seed, classifierTag)

% X_val: [N,T,C], Y_val: [N,1]
% 仅 forward，不求梯度，不调用重建模块
...
N = size(X_val,1);
C = size(X_val,3);
K_max_scan = min(max(1, round(K_max_scan)), C);

[~, acc0] = evalClassifierCEAcc(classifierNet, X_val, Y_val);

mean_drop = zeros(K_max_scan, 1);
worst_drop = zeros(K_max_scan, 1);
std_drop = zeros(K_max_scan, 1);
mean_acc = zeros(K_max_scan, 1);

for k = 1:K_max_scan
    drops = zeros(num_trials, 1);
    accs = zeros(num_trials, 1);

    for tr = 1:num_trials
        S = randperm(C, k);  % uniform sampling channels

        X_corrupt = X_val;
        X_corrupt(:,:,S) = 0;  % 与 generate_channels 的异常置零方式一致

        [~, acc_k] = evalClassifierCEAcc(classifierNet, X_corrupt, Y_val);

        accs(tr) = acc_k;
        drops(tr) = acc0 - acc_k;
    end

    mean_drop(k) = mean(drops);
    worst_drop(k) = max(drops);
    std_drop(k) = std(drops);
    mean_acc(k) = mean(accs);

    fprintf('Critical-K scan | K=%d/%d, mean_drop=%.4f, worst_drop=%.4f, std_drop=%.4f\n', ...
        k, K_max_scan, mean_drop(k), worst_drop(k), std_drop(k));
end

idx_mean = find(mean_drop >= drop_threshold, 1, 'first');
if isempty(idx_mean), idx_mean = NaN; end

idx_worst = find(worst_drop >= drop_threshold, 1, 'first');
if isempty(idx_worst), idx_worst = NaN; end

Critical_K = idx_mean;
Critical_K_worst = idx_worst;

if isnan(Critical_K)
    K_max_rec = min(ceil(C * 0.30), 6);
else
    K_max_rec = min(ceil(C * 0.30), 2 * Critical_K);
end
K_max_rec = max(1, min(C, round(K_max_rec)));

acc_curve = [mean_drop, worst_drop, std_drop];  % K x 3

meta = struct();
meta.drop_threshold = drop_threshold;
meta.num_trials = num_trials;
meta.seed = seed;
meta.timestamp = datestr(now, 31);
meta.classifier_tag = classifierTag;
meta.acc0 = acc0;
meta.K_max_scan = K_max_scan;
meta.N = N;
meta.C = C;

criticalInfo = struct();
criticalInfo.Critical_K = Critical_K;
criticalInfo.Critical_K_worst = Critical_K_worst;
criticalInfo.K_max_rec = K_max_rec;
criticalInfo.acc_curve = acc_curve;
criticalInfo.mean_acc = mean_acc;
criticalInfo.meta = meta;
...
% 图：Accuracy(K) + drop曲线（含阈值线）
fig = figure('Name', 'Critical-K Diagnostic', 'Color', 'w');
tiledlayout(fig, 1, 2, 'Padding', 'compact', 'TileSpacing', 'compact');

ax1 = nexttile;
K_axis = (1:K_max_scan)';
errorbar(ax1, K_axis, mean_acc, std_drop, 'b-o', 'LineWidth', 1.2); % 误差条近似用drop std
grid(ax1, 'on');
xlabel(ax1, 'K');
ylabel(ax1, 'Accuracy');
title(ax1, 'Accuracy(K)');

ax2 = nexttile;
plot(ax2, K_axis, mean_drop, 'r-o', 'LineWidth', 1.5); hold(ax2, 'on');
plot(ax2, K_axis, worst_drop, 'm--s', 'LineWidth', 1.2);
yline(ax2, drop_threshold, 'k-.', 'LineWidth', 1.2);
grid(ax2, 'on');
xlabel(ax2, 'K');
ylabel(ax2, 'Acc Drop');
title(ax2, 'Drop(K)');
legend(ax2, {'mean drop','worst drop','threshold'}, 'Location', 'best');

saveas(fig, savePngPath);
